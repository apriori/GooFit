  ...
  , workSpaceIndex(0)
{
  ...
  if (0 == numOthers) paramIndices.push_back(workSpaceIndex); 
  else {
...
__host__ void ConvolutionPdf::registerOthers (std::vector<ConvolutionPdf*> others) {
  ...
  for (unsigned int i = 0; i < others.size(); ++i) {
    ...
    if (curr == this) foundSelf = true;
    host_indices[parameters + 8 + i] = curr->workSpaceIndex; 
  }

  if (!foundSelf) {
    std::cout << "Problem: " << getName() << " initialised with list that did not include itself. Returning without initialisation.\n";
    return;
  }
